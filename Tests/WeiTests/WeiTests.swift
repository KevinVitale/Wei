...
final class WeiTests: XCTestCase {
  ...
  func testMaxWei() {
    let balanceAsHex = "0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff7"
    XCTAssertEqual(balanceAsHex, balanceAsHex(as: .wei).hexString)
    XCTAssertEqual(balanceAsHex, Wei.max.hexString)
    XCTAssertEqual(balanceAsHex, Wei.max.to(.ether).hexString)
    XCTAssertEqual(
      "115792089237316195423570985008687907853269984665640564039457584007913129639927 wei",
      balanceAsHex(as: .wei).debugDescription
    )
  }
