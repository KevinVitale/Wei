...
public struct Wei: Codable, Equatable, CustomDebugStringConvertible, ExpressibleByIntegerLiteral, ExpressibleByStringLiteral, ExpressibleByFloatLiteral {
    ...
    public var hexString :String {
        guard let bigInt = BigInt(to(.wei).wrappedValue) else {
            return "0x"
        }
...
extension StringProtocol {
    ...
    fileprivate func stripHexIfNecessary() -> Self {
        if starts(with: "0x"), let bigInt = BigInt(dropFirst(2), radix: 16), let description = bigInt.description as? Self {
            return description
        } else {
            return self
        }
...
extension Wei {
    public enum Denomination: Comparable, CodingKey {
        ...
        static let allKeys: [Denomination] = [
            .wei,
            .kwei,
            .mwei,
            .gwei,
